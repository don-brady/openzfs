...
#
log_must dd if=/dev/zero of=/$TESTPOOL1/fs/sync \
...
log_note "Starting zpool import in background at" $(/usr/bin/date +'%H:%M:%S:%3N')
...
while [[ -d /proc/"$pid" ]]; do
	line=$(cat /proc/spl/kstat/zfs/import_progress | grep -v pool_guid)
	if [[ -n $line ]]; then
		echo $(/usr/bin/date +'%H:%M:%S:%3N') $line
	fi
...
log_note "zpool import completed at" $(/usr/bin/date +'%H:%M:%S:%3N')

entries=$(grep "spa_import_progress_set_notes(): 'testpool1'" /proc/spl/kstat/zfs/dbgmsg | wc -l)
